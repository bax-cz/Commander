...
static BTKey *catalogKeyRead(size_t offset, io_func *io) {
  HFSPlusCatalogKey *key;
  uint16_t i;

  key = (HFSPlusCatalogKey *)malloc(sizeof(HFSPlusCatalogKey));

  if (!HFS_READ(io, offset, UNICODE_START, key)) {
    return NULL;
  }
  ...
  if (!HFS_READ(io, offset + UNICODE_START, key->nodeName.length * sizeof(uint16_t),
...
static int catalogKeyWrite(size_t offset, BTKey *toWrite, io_func *io) {
  HFSPlusCatalogKey *key;
  uint16_t i;
  uint16_t keyLength;
  ...
  if (!HFS_WRITE(io, offset, keyLength, key)) {
    return FALSE;
  }
...
static BTKey *catalogDataRead(size_t offset, io_func *io) {
  int16_t recordType;
  HFSPlusCatalogRecord *record;
  uint16_t nameLength;

  if (!HFS_READ(io, offset, sizeof(int16_t), &recordType)) {
    return NULL;
  }
  ...
  case kHFSPlusFolderRecord:
    ...
    if (!HFS_READ(io, offset, sizeof(HFSPlusCatalogFolder), record)) {
      return NULL;
    }
  ...
  case kHFSPlusFileRecord:
    ...
    if (!HFS_READ(io, offset, sizeof(HFSPlusCatalogFile), record)) {
      return NULL;
    }
...
CatalogRecordList *getFolderContents(HFSCatalogNodeID CNID, Volume *volume) {
  ...
  while (nodeNumber != 0) {
    ...
    while (recordNumber < descriptor->numRecords) {
      ...
      currentKey = (HFSPlusCatalogKey *)HFS_READ_KEY(tree, recordOffset, tree->io);
      recordDataOffset =
      ...
      if (currentKey->parentID == CNID) {
        ...
        item->record =
            (HFSPlusCatalogRecord *)HFS_READ_DATA(tree, recordDataOffset, tree->io);
...
HFSPlusCatalogRecord *getLinkTarget(HFSPlusCatalogRecord *record,
  ...
  if (record->recordType == kHFSPlusFileRecord &&
    ...
    io = openRawFile(((HFSPlusCatalogFile *)record)->fileID,
                     ...
                     volume);
    HFS_READ(io, 0, (((HFSPlusCatalogFile *)record)->dataFork).logicalSize,
         pathBuffer);
    HFS_CLOSE(io);
...
int makeSymlink(const char *pathName, const char *target, Volume *volume) {
  ...
  HFS_WRITE(io, 0, strlen(target), (void *)target);
  HFS_CLOSE(io);
  free(record);

...
static HFSPlusVolumeHeader *readVolumeHeader(io_func *io, size_t offset) {
  HFSPlusVolumeHeader *volumeHeader;

  volumeHeader = (HFSPlusVolumeHeader *)malloc(sizeof(HFSPlusVolumeHeader));

  if (!(HFS_READ(io, offset, sizeof(HFSPlusVolumeHeader), volumeHeader)))
    return NULL;
...
static int writeVolumeHeader(io_func *io,
  ...
  if (!(HFS_WRITE(io, offset, sizeof(HFSPlusVolumeHeader), volumeHeader))) {
    return FALSE;
  }
...
int updateVolume(Volume *volume) {
  ASSERT(writeVolumeHeader(volume->image, volume->volumeHeader,
                           ((size_t)volume->volumeHeader->totalBlocks *
                            (size_t)volume->volumeHeader->blockSize) -
                               1024),
         "writeVolumeHeader");
...
void closeVolume(Volume *volume) {
  ...
  HFS_CLOSE(volume->allocationFile);
  closeBTree(volume->catalogTree);
  closeBTree(volume->extentsTree);
  free(volume->volumeHeader);
  free(volume);

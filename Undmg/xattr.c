...
static BTKey *attrKeyRead(size_t offset, io_func *io) {
  int i;
  HFSPlusAttrKey *key;
  ...
  if (!HFS_READ(io, offset + UNICODE_START, key->name.length * sizeof(uint16_t),
...
static int attrKeyWrite(size_t offset, BTKey *toWrite, io_func *io) {
  HFSPlusAttrKey *key;
  uint16_t keyLength;
...
static BTKey *attrDataRead(size_t offset, io_func *io) {
  HFSPlusAttrRecord *record;

  record = (HFSPlusAttrRecord *)malloc(sizeof(HFSPlusAttrRecord));

  if (!HFS_READ(io, offset, sizeof(uint32_t), record)) {
    return NULL;
  }
  ...
  case kHFSPlusAttrInlineData:
    if (!HFS_READ(io, offset, sizeof(HFSPlusAttrData), record)) {
      return NULL;
    }
    ...
    if (!HFS_READ(io, offset + sizeof(HFSPlusAttrData),
              ((HFSPlusAttrData *)record)->size,
  ...
  case kHFSPlusAttrForkData:
    if (!HFS_READ(io, offset, sizeof(HFSPlusAttrForkData), record)) {
      return NULL;
    }
  ...
  case kHFSPlusAttrExtents:
    if (!HFS_READ(io, offset, sizeof(HFSPlusAttrExtents), record)) {
      return NULL;
    }
...
XAttrList *getAllExtendedAttributes(HFSCatalogNodeID CNID, Volume *volume) {
  ...
  while (nodeNumber != 0) {
    ...
    while (recordNumber < descriptor->numRecords) {
      ...
      currentKey = (HFSPlusAttrKey *)HFS_READ_KEY(tree, recordOffset, tree->io);

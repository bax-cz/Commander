...
#include "dirent.h" /* windows version of dirent */
...
void writeToFile(HFSPlusCatalogFile *file, AbstractFile *output,
  ...
  while (bytesLeft > 0) {
    if (bytesLeft > BUFSIZE) {
      if (!HFS_READ(io, curPosition, BUFSIZE, buffer)) {
      ...
      if (!HFS_READ(io, curPosition, bytesLeft, buffer)) {
        hfs_panic("error reading");
      }
...
void writeToHFSFile(HFSPlusCatalogFile *file, AbstractFile *input,
  ...
  while (bytesLeft > 0) {
    if (bytesLeft > BUFSIZE) {
      ...
      if (!HFS_WRITE(io, curPosition, BUFSIZE, buffer)) {
        hfs_panic("error writing");
      }
      curPosition += BUFSIZE;
      bytesLeft -= BUFSIZE;
      ...
      if (!HFS_WRITE(io, curPosition, (size_t)bytesLeft, buffer)) {
        hfs_panic("error reading");
      }
      curPosition += bytesLeft;
      bytesLeft -= bytesLeft;
...
void grow_hfs(Volume *volume, uint64_t newSize) {
  ...
  for (i = ((volume->volumeHeader->totalBlocks / 8) + 1); i < newMapSize; i++) {
    ASSERT(HFS_WRITE(volume->allocationFile, i, 1, &zero), "WRITE");
  }

  /* grow backing store size */
  ASSERT(HFS_WRITE(volume->image, newSize - 1, 1, &zero), "WRITE");
...
void addAllInFolder(HFSCatalogNodeID folderID, Volume *volume,
  ...
  ASSERT(_getcwd(cwd, 1024) != NULL, "cannot get current working directory");
  ...
  while ((ent = readdir(dir)) != NULL) {
    ...
    if ((tmp = opendir(ent->d_name)) != NULL) {
      ...
      fullName[pathLen] = '/';
      fullName[pathLen + 1] = '\0';
      ASSERT(_chdir(ent->d_name) == 0, "chdir");
      addAllInFolder(cnid, volume, fullName);
      ASSERT(_chdir(cwd) == 0, "chdir");
    } else {
      if (cnid == 0) {
  ...
  while (list != NULL) {
    ...
    if (list->record->recordType == kHFSPlusFolderRecord) {
      ...
      if (_stat(name, &status) != 0) {
        ASSERT(_mkdir(name/*, 0755*/) == 0, "mkdir");
      }
      ASSERT(_chdir(name) == 0, "chdir");
      ...
      fd = _open(name, _O_CREAT | _O_WRONLY | _O_BINARY, file->permissions.fileMode);
      outFile = createAbstractFileFromFile(_fdopen(fd, "wb"));
      if (outFile != NULL) {

...
int fseekWrapper(AbstractFile *file, size_t offset) {
  return _fseeki64((FILE *)(file->data), offset, SEEK_SET);
}

size_t ftellWrapper(AbstractFile *file) { return _ftelli64((FILE *)(file->data)); }
...
size_t fileGetLength(AbstractFile *file) {
  size_t length;
  size_t pos;

  pos = _ftelli64((FILE *)(file->data));

  _fseeki64((FILE *)(file->data), 0, SEEK_END);
  length = _ftelli64((FILE *)(file->data));

  _fseeki64((FILE *)(file->data), pos, SEEK_SET);
...
size_t dummyWrite(AbstractFile *file, const void *data, size_t len) {
  *((size_t *)(file->data)) += len;
  return len;
}

int dummySeek(AbstractFile *file, size_t offset) {
  *((size_t *)(file->data)) = offset;
  return 0;
}

size_t dummyTell(AbstractFile *file) { return *((size_t *)(file->data)); }
...
int memSeek(AbstractFile *file, size_t offset) {
  MemWrapperInfo *info = (MemWrapperInfo *)(file->data);
  info->offset = (size_t)offset;
  return 0;
}

size_t memTell(AbstractFile *file) {
  MemWrapperInfo *info = (MemWrapperInfo *)(file->data);
  return (size_t)info->offset;
}

size_t memGetLength(AbstractFile *file) {
  MemWrapperInfo *info = (MemWrapperInfo *)(file->data);
...
int absFileRead(io_func *io, size_t location, size_t size, void *buffer) {
  AbstractFile *file;
...
int absFileWrite(io_func *io, size_t location, size_t size, void *buffer) {
  AbstractFile *file;
  file = (AbstractFile *)io->data;
  file->seek(file, location);
...
int memFileSeek(AbstractFile *file, size_t offset) {
  MemFileWrapperInfo *info = (MemFileWrapperInfo *)(file->data);
  info->offset = (size_t)offset;
  return 0;
}

size_t memFileTell(AbstractFile *file) {
  MemFileWrapperInfo *info = (MemFileWrapperInfo *)(file->data);
  return (size_t)info->offset;
}

size_t memFileGetLength(AbstractFile *file) {
  MemFileWrapperInfo *info = (MemFileWrapperInfo *)(file->data);
  return *(info->bufferSize);
}

...
static BTKey *extentKeyRead(size_t offset, io_func *io) {
  HFSPlusExtentKey *key;

  key = (HFSPlusExtentKey *)malloc(sizeof(HFSPlusExtentKey));

  if (!HFS_READ(io, offset, sizeof(HFSPlusExtentKey), key))
    return NULL;
...
static int extentKeyWrite(size_t offset, BTKey *toWrite, io_func *io) {
  HFSPlusExtentKey *key;
  ...
  if (!HFS_WRITE(io, offset, sizeof(HFSPlusExtentKey), key))
    return FALSE;
...
static BTKey *extentDataRead(size_t offset, io_func *io) {
  HFSPlusExtentRecord *record;

  record = (HFSPlusExtentRecord *)malloc(sizeof(HFSPlusExtentRecord));

  if (!HFS_READ(io, offset, sizeof(HFSPlusExtentRecord), record))
    return NULL;

...
static int compressedRead(io_func *io, size_t location, size_t size,
                          void *buffer) {
  ...
  while (size > 0) {
    ...
    if (!HFS_READ(data->io, data->rsrcHead.headerSize + sizeof(uint32_t) +
...
static int compressedWrite(io_func *io, size_t location, size_t size,
                           void *buffer) {
  HFSPlusCompressed *data = (HFSPlusCompressed *)io->data;
  ...
  if (data->dirty) {
    ...
    for (i = 0; i < numBlocks; i++) {
      ...
      if (numBlocks <= 1 &&
        ...
        HFS_WRITE(data->io, data->rsrcHead.headerSize + sizeof(uint32_t) +
    ...
    if (data->decmpfs->flags == 0x4) {
      ...
      HFS_WRITE(data->io, 0, sizeof(HFSPlusCmpfRsrcHead), &data->rsrcHead);
      flipRsrcHead(&data->rsrcHead);
      ...
      HFS_WRITE(data->io, data->rsrcHead.headerSize, blocksSize, data->blocks);
      flipRsrcBlockHead(data->blocks);
      for (i = 0; i < data->blocks->numBlocks; i++) {
        flipRsrcBlock(&data->blocks->blocks[i]);
      }
      ...
      HFS_WRITE(data->io, data->rsrcHead.totalSize, sizeof(HFSPlusCmpfEnd), &end);
      flipHFSPlusCmpfEnd(&end);
...
io_func *openHFSPlusCompressed(Volume *volume, HFSPlusCatalogFile *file) {
  ...
  if (data->decmpfs->flags == 0x3) {
    ...
    if (!HFS_READ(data->io, 0, sizeof(HFSPlusCmpfRsrcHead), &data->rsrcHead)) {
      hfs_panic("error reading");
    }
    ...
    if (!HFS_READ(data->io, data->rsrcHead.headerSize,

...
int isBlockUsed(Volume *volume, uint32_t block) {
  ...
  HFS_READ(volume->allocationFile, block / 8, 1, &byte);
...
int setBlockUsed(Volume *volume, uint32_t block, int used) {
  ...
  ASSERT(HFS_WRITE(volume->allocationFile, block / 8, 1, &byte), "WRITE");
  ...
  if (blocksNeeded > forkData->totalBlocks) {
    ...
    while (blocksToAllocate > 0) {
      if (isBlockUsed(volume, curBlock)) {
        ...
        ASSERT(HFS_WRITE(volume->image, curBlock * volume->volumeHeader->blockSize,
...
static int rawFileRead(io_func *io, size_t location, size_t size, void *buffer) {
  RawFile *rawFile;
  Volume *volume;
  Extent *extent;
...
static int rawFileWrite(io_func *io, size_t location, size_t size,
                        void *buffer) {
  RawFile *rawFile;
  Volume *volume;
  Extent *extent;

...
#ifdef _DEBUG
#define DebugString(x) OutputDebugString( _T(x) )
#else
...
{
	_ASSERTE(file && file != INVALID_HANDLE_VALUE);
...
static LONGLONG SetFilePos(HANDLE file, LONGLONG pos, DWORD method = FILE_BEGIN)
{
	_ASSERTE(file && file != INVALID_HANDLE_VALUE);
	...
	if (SetFilePointerEx(file, p, &ret, method))
		return ret.QuadPart;
...
{
	_ASSERTE(file != 0 && file != INVALID_HANDLE_VALUE);
	_ASSERTE(data);
	...
	DWORD startblk = 0, bool sharp = false)
{
		...
		i++, step1 = sharp, iterations++)
	{
			...
			descriptor->VolumeDescriptor.VolumeDescriptorType == 1)
		{
			...
			_ASSERTE(sizeof(BootRecordVolumeDescriptor) == 0x800);
			_ASSERTE(sizeof(InitialEntry) == 0x20);
			_ASSERTE(sizeof(SectionHeaderEntry) == 0x20);
			_ASSERTE(sizeof(SectionEntry) == 0x20);
			_ASSERTE(sizeof(SectionEntryExtension) == 0x20);
							...
							(short unsigned int)catalog.Validation.KeyWord == (short unsigned int)0xaa55)
						{
							...
							else
							{
								...
								{ // reading images
									...
									_ASSERTE(descriptor->BootCatalog);
									if (catalog.Entry[0].Initial.Bootable == 0x88)
									{
...
static IsoImage* ReadUDFStructure(IsoImage* image)
{
	...
	image->DescriptorNum++;
	image->VolumeDescriptors = (PrimaryVolumeDescriptorEx*)realloc(image->VolumeDescriptors, sizeof(*image->VolumeDescriptors) * image->DescriptorNum);
	ZeroMemory(image->VolumeDescriptors, sizeof(*image->VolumeDescriptors) * image->DescriptorNum);
...
static IsoImage* GetImage(const wchar_t* wfilename = 0)
{
	...
	_ASSERTE(sizeof(PrimaryVolumeDescriptor) == 0x800); // check for size of descriptor
	DWORD read;
	...
	Directory** dirs, DWORD* count, bool boot = false)
{
		...
		{ // if dirs == 0 - we just calculating total number of records...
			if (desc->Unicode && record->LengthOfFileIdentifier > 1 && num > 1)
			{
				...
				if (!is_utf8(record->FileIdentifier, record->LengthOfFileIdentifier))
				{
					...
					directory.FileName = (char*)calloc((length + 1), sizeof(*directory.FileName));
					directory.wFileName = (wchar_t*)calloc((length + 1), sizeof(*directory.wFileName));
					_ASSERTE(directory.FileName);
					_ASSERTE(directory.wFileName);
				...
				{ // hmmm... it seems here is Joliet extension where UTF-8 codepage used :)
					directory.FileName = (char*)calloc((record->LengthOfFileIdentifier + 2), sizeof(*directory.FileName));
					_ASSERTE(directory.FileName);
					if (!directory.FileName)
					{
						free(data);
				...
				int dirlen = lstrlenA(path) + 1 + lstrlenA(directory.FileName) + 1;
				directory.FilePath = (char*)calloc(dirlen, sizeof(*directory.FilePath));
				_ASSERTE(directory.FilePath);
				if (!directory.FilePath)
				{
					free(data);
				...
				if (wpath)
				{
					...
					directory.wFilePath = (wchar_t*)calloc(wdirlen, sizeof(*directory.wFilePath));
					_ASSERTE(directory.wFilePath);
	...
	if (desc->BootImageEntries && boot)
	{
		if (dirs)
		{
			...
			dir->FileName = (char*)calloc(len, sizeof(char));
			dir->FilePath = (char*)calloc(len, sizeof(char));
...
static tChangeVolProcW   ChangeVolProcW;
static tProcessDataProcW ProcessDataProcW;

static bool ExtractFile(IsoImage* image, Directory* directory, const wchar_t* wpath = 0, bool append = false)
{
	_ASSERTE(image && directory && wpath);
	...
	if (!buffer)
	{
		_ASSERTE("can't allocate memory for file extract");
		return false;
	}
	...
	for (; (int)size >= 0; size -= sector, block += block_increment)
	{
		...
		{
			if (wpath)
				file = CreateFileW(wpath, GENERIC_WRITE, FILE_SHARE_READ, 0, append ? OPEN_EXISTING : CREATE_ALWAYS,
			...
			if (append)
				SetFilePos(file, 0ll, FILE_END);
		}

		if (!ProcessDataProcW(image->DirectoryList[image->Index].wFileName, // check for user pressed "Cancel"
			(int)cur_size, image->UserData))
		{
			CloseHandle(file);
	...
	if (file)
	{
		if (!xbox)
		{
			...
			SystemTimeToFileTime(&time, &dtime);
...
HANDLE __stdcall IsoOpenArchiveW(tOpenArchiveDataW* ArchiveData)
{
...
int __stdcall IsoReadHeaderExW(HANDLE hArcData, tHeaderDataExW* HeaderData)
{
	Directory* directory;
...
int __stdcall IsoProcessFileW(HANDLE hArcData, int Operation, const wchar_t* DestPath, const wchar_t* DestName)
{
	...
	if (Operation == PK_EXTRACT)
	{
		...
		if (image->Index + 1 < image->DirectoryCount)
		{
			result = ExtractFile(image, &image->DirectoryList[image->Index], path);

			// concatenate partial files into one
			while((image->DirectoryList[image->Index].wFilePath && image->DirectoryList[image->Index].wFileName // unicode paths
				&& image->DirectoryList[image->Index + 1].wFilePath && image->DirectoryList[image->Index + 1].wFileName
				&& (wcscmp(image->DirectoryList[image->Index].wFilePath, image->DirectoryList[image->Index + 1].wFilePath) == 0)
				&& (wcscmp(image->DirectoryList[image->Index].wFileName, image->DirectoryList[image->Index + 1].wFileName) == 0))
				||(image->DirectoryList[image->Index].FilePath && image->DirectoryList[image->Index].FileName // ansi paths
				&& image->DirectoryList[image->Index + 1].FilePath && image->DirectoryList[image->Index + 1].FileName
				&& (strcmp(image->DirectoryList[image->Index].FilePath, image->DirectoryList[image->Index + 1].FilePath) == 0)
				&& (strcmp(image->DirectoryList[image->Index].FileName, image->DirectoryList[image->Index + 1].FileName) == 0)))
			{
				image->Index++;

				// partial file
				result = ExtractFile(image, &image->DirectoryList[image->Index], path, true);
...
int __stdcall IsoCloseArchive(HANDLE hArcData)
{
...
void __stdcall IsoSetChangeVolProcW(HANDLE /*hArcData*/, tChangeVolProcW pChangeVolProc)
{
	ChangeVolProcW = pChangeVolProc;
}

void __stdcall IsoSetProcessDataProcW(HANDLE hArcData, tProcessDataProcW pProcessDataProc, LPARAM pUserData)
{
	if (hArcData && pProcessDataProc)
	{
		IsoImage* image = (IsoImage*)hArcData;
		image->UserData = pUserData;
		ProcessDataProcW = pProcessDataProc;
	}
}

BOOL __stdcall IsoCanYouHandleThisFileW(const wchar_t* FileName)
{
	if (!FileName)
		return FALSE;
...
int __stdcall IsoGetPackerCaps()
{

...
*/
int mkd2html(const wchar_t *inname, const wchar_t *outname)
{
    char *h;
	...
	if ( (input = _wfopen(inname, L"r")) == 0 )
		return 1;//fail("can't open either %s or %s", argv[0], source);

	if ( (output = _wfopen(outname, L"w")) == 0 ) {
		fclose(input);
		return 2;//fail("can't write to %s", dest);
	}

	mmiot = gfm ? gfm_in(input, 0) : mkd_in(input, 0);

	if ( mmiot == 0 )
		return 3;//fail("can't read %s", source ? source : "stdin");

	if ( !mkd_compile(mmiot, 0) )
		return 4;//fail("couldn't compile input");

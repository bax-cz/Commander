...
namespace nsFTP
{
	...
	public:
		...
		bool ListAll(const tstring& strPath, TStringVector& vstrFileList, bool fPasv = false) const;
		...
		bool ListAll(const tstring& strPath, TFTPFileStatusShPtrVec& vFileList, bool fPasv = false) const;
		...
		int RemoveDirectoryFtp(const tstring& strDirectory) const; /* JJ: renamed because of WIN32 RemoveDirectory macro */
		int MakeDirectoryFtp(const tstring& strDirectory) const;
		...
		int System() const;
		int Noop() const;
		int Feat(TStringVector& vstrFeaturesList) const;
		int RepresentationType(const CRepresentation& repType, size_t dwSize = 0) const;
		...
		int Restart(size_t dwPosition) const;

		int FileSize(const tstring& strPath, long long& llSize) const;
		int FileChecksum(const tstring& strPath, DWORD& crc32) const;
	...
	private:
		CFTPClient& operator=(const CFTPClient&) = delete; // no implementation for assignment operator
		int _RepresentationType(const CRepresentation& repType, size_t dwSize = 0) const;
		bool TransferData(const CCommand& crDatachannelCmd, ITransferNotification& Observer, IBlockingSocket& sckDataConnection) const;
		bool OpenActiveDataConnection(IBlockingSocket& sckDataConnection, const CCommand& crDatachannelCmd, const tstring& strPath, size_t dwByteOffset) const;
		bool OpenPassiveDataConnection(IBlockingSocket& sckDataConnection, const CCommand& crDatachannelCmd, const tstring& strPath, size_t dwByteOffset) const;
	...
	private:
		...
		bool                                     m_fUtf8;                    ///< force Unicode UTF-8 text encoding
		...
		mutable nsHelper::CTimer                 m_Timer;                    ///< timer object, the time at which the last command has been sent
	};

	/// @brief interface for notification
	///
	/// Derive your class from this base-class and register this class on CFTPClient.
	/// For example you can use this for logging the sent and received commands.
	...
	public:
		...
		virtual void OnEndReceivingData(long long /*llReceivedBytes*/) {}
		...
		virtual void OnPreReceiveFile(const tstring& /*strSourceFile*/, const tstring& /*strTargetFile*/, long long /*llFileSize*/) {}
		virtual void OnPreSendFile(const tstring& /*strSourceFile*/, const tstring& /*strTargetFile*/, long long /*llFileSize*/) {}
		virtual void OnPostReceiveFile(const tstring& /*strSourceFile*/, const tstring& /*strTargetFile*/, long long /*llFileSize*/) {}
		virtual void OnPostSendFile(const tstring& /*strSourceFile*/, const tstring& /*strTargetFile*/, long long /*llFileSize*/) {}
	...
	public:
		...
		virtual size_t GetLocalStreamSize() const = 0;
		virtual void SetLocalStreamOffset(size_t dwOffsetFromBeginOfStream) = 0;
		virtual void OnBytesReceived(const TByteVector& /*vBuffer*/, long /*lReceivedBytes*/) {}
	...
	{
		class CPimpl;
		std::shared_ptr<CPimpl> m_spPimpl;
		bool m_bUtf8;
	public:
		COutputStream(const tstring& strEolCharacterSequence, const tstring& strStreamName, bool utf8 = false);

...
// History:
// 1.6 (2022-06-03) JJ:
//      - auto_ptr changed to unique_ptr
//      - 64-bit files support added
//      - ListAll function (LIST -a)
//      - KeepAlive method added
//      - FEAT command added
//      - XCRC non-standard command added
//      - UTF-8 support added (only WIN32 tested)
...
namespace nsFTP
{
	...
	{
	public:
		CMakeString& operator<<(size_t dwNum)
		{
			size_t dwTemp = dwNum;
			...
			tsprintf(&(*m_str.begin()), m_str.size() + 1, _T("%ls%zu"), m_str.c_str(), dwNum);
...
#if _MSC_VER >= 1500
# ifdef _UNICODE
			return _wfopen_s(&m_pFile, CCnv::MakeLongPath(strFileName).c_str(), strMode.c_str()) == 0;
# else
			return fopen_s(&m_pFile, CCnv::MakeLongPath(strFileName).c_str(), strMode.c_str()) == 0;
# endif
#else
# ifdef _UNICODE
			m_pFile = _wfopen(strFileName.c_str(), strMode.c_str());
# else
...
# endif
		...
		bool Seek(long long llOffset, TOriginEnum enOrigin)
		{
			return m_pFile && _fseeki64(m_pFile, llOffset, enOrigin) == 0;
		}
		...
		virtual size_t GetLocalStreamSize() const override
		{
			if (!m_pFile)
				return 0;

			const long long llCurPos = _ftelli64(m_pFile);
			_fseeki64(m_pFile, 0, SEEK_END);
			const long long llEndPos = _ftelli64(m_pFile);
			_fseeki64(m_pFile, llCurPos, SEEK_SET);

			return (size_t)llEndPos;
		}

		virtual void SetLocalStreamOffset(size_t dwOffsetFromBeginOfStream) override
		{
	...
	{
		CPimpl& operator=(const CPimpl&) = delete; // no implementation for assignment operator
	public:
	...
	COutputStream::COutputStream(const tstring& strEolCharacterSequence, const tstring& strStreamName, bool utf8) :
	...
	bool COutputStream::GetNextLine(tstring& strLine)
	{
	...
	size_t COutputStream::GetLocalStreamSize() const
	{
		return m_spPimpl->m_vBuffer.size();
	}

	void COutputStream::SetLocalStreamOffset(size_t dwOffsetFromBeginOfStream)
	{
	...
	void COutputStream::OnBytesReceived(const TByteVector& vBuffer, long lReceivedBytes)
	{
		if (m_bUtf8) {
			tstring strOut;
			CCnv::ConvertToTString(&vBuffer[0], lReceivedBytes + 1, strOut, true);
			m_spPimpl->m_vBuffer += strOut;
		}
		else
...
CFTPClient::CFTPClient(std::unique_ptr<IBlockingSocket> apSocket, unsigned int uiTimeout/*=10*/,
	...
	m_fUtf8(false)
{
	ASSERT(m_apSckControlConnection != nullptr);
}
...
/// Keeps the connection to the FTP server alive by sending the NOOP command.
void CFTPClient::KeepAlive() const
{
	if (IsConnected() && m_Timer.GetElapsedTime() > 30.0)
		Noop();
}
...
/// Analyse the reply code of a FTP server-response.
...
bool CFTPClient::Login(const CLogonInfo& logonInfo)
{
	...
	// check for utf-8 feature
	TStringVector feats;
	if (Feat(feats) == FTP_OK)
	{
		if (std::find_if(feats.begin(), feats.end(), [](const tstring& feat) { return feat.find(L"UTF8") != tstring::npos; }) != feats.end())
			m_fUtf8 = true;
	}
...
/// Gets the features list supported by the FTP server. Sends the FEAT command to the FTP server.
/// @param[out] vstrFeaturesList Returns a simple list of the supported features.
int CFTPClient::Feat(TStringVector& vstrFeaturesList) const
{
	CReply Reply;
	if (!SendCommand(CCommand::FEAT(), CArg(), Reply))
		return FTP_ERROR;

	vstrFeaturesList.clear();

	if (Reply.Code().IsPositiveCompletionReply())
	{
		COutputStream outputStream(mc_strEolCharacterSequence, CCommand::FEAT().AsString());
		outputStream.SetBuffer(Reply.Value());
		outputStream.SetStartPosition();
		tstring strLine;
		while (outputStream.GetNextLine(strLine))
			vstrFeaturesList.push_back(strLine.c_str());
	}
...
{
	COutputStream outputStream(mc_strEolCharacterSequence, CCommand::LIST().AsString(), m_fUtf8);
...
/// Gets the directory listing (including hidden files/directories) of the FTP server.
/// Sends the LIST -a command to the FTP server.
/// @param[in] strPath Starting path for the list command.
/// @param[out] vstrFileList Returns a simple list of the files and folders of the specified directory.
/// @param[in] fPasv see documentation of CFTPClient::Passive
bool CFTPClient::ListAll(const tstring& strPath, TStringVector& vstrFileList, bool fPasv) const
{
	COutputStream outputStream(mc_strEolCharacterSequence, CCommand::LSTA().AsString(), m_fUtf8);
	if (!ExecuteDatachannelCommand(CCommand::LSTA(), strPath, CRepresentation(CType::ASCII()), fPasv, 0, outputStream))
		return false;
...
{
	COutputStream outputStream(mc_strEolCharacterSequence, CCommand::NLST().AsString(), m_fUtf8);
...
/// Gets the directory listing (including hidden files/directories) of the FTP server.
/// Sends the non-standard LIST -a command to the FTP server.
/// @param[in] strPath Starting path for the list command.
/// @param[out] vFileList Returns a detailed list of the files and folders of the specified directory.
///                       vFileList contains CFTPFileStatus-Objects. These Objects provide a lot of
///                       information about the file/folder.
/// @param[in] fPasv see documentation of CFTPClient::Passive
bool CFTPClient::ListAll(const tstring& strPath, TFTPFileStatusShPtrVec& vFileList, bool fPasv) const
{
	ASSERT(m_apFileListParser.get());
...
bool CFTPClient::DownloadFile(const tstring& strRemoteFile, const tstring& strLocalFile, const CRepresentation& repType, bool fPasv) const
{
	...
	if (!file.Open(strLocalFile, m_fResumeIfPossible ? _T("ab") : _T("wb")))
	{
		...
		ReportError(strLocalFile + L"\r\n" + CError::GetErrorDescription(id), CCnv::ConvertToTString(__FILE__), __LINE__);
		return false;
	}
...
{
	long long llRemoteFileSize = 0;
	FileSize(strRemoteFile, llRemoteFileSize);

	for (TObserverSet::const_iterator it = m_setObserver.begin(); it != m_setObserver.end(); it++)
		(*it)->OnPreReceiveFile(strRemoteFile, Observer.GetLocalStreamName(), llRemoteFileSize);
	...
	for (TObserverSet::const_iterator it2 = m_setObserver.begin(); it2 != m_setObserver.end(); it2++)
		(*it2)->OnPostReceiveFile(strRemoteFile, Observer.GetLocalStreamName(), llRemoteFileSize);
...
bool CFTPClient::UploadFile(ITransferNotification& Observer, const tstring& strRemoteFile, bool fStoreUnique, const CRepresentation& repType, bool fPasv) const
{
	...
	if (llRemoteFileSize > 0)
		cmd = CCommand::APPE();
	else if (fStoreUnique)
		cmd = CCommand::STOU();

	const long long llLocalFileSize = Observer.GetLocalStreamSize(); // 32-bit size limit on 32-bit platforms
	Observer.SetLocalStreamOffset((size_t)llRemoteFileSize);

	TObserverSet::const_iterator it;
	for (it = m_setObserver.begin(); it != m_setObserver.end(); it++)
		(*it)->OnPreSendFile(Observer.GetLocalStreamName(), strRemoteFile, llLocalFileSize);

	const bool fRet = ExecuteDatachannelCommand(cmd, strRemoteFile, repType, fPasv, 0, Observer);

	for (it = m_setObserver.begin(); it != m_setObserver.end(); it++)
		(*it)->OnPostSendFile(Observer.GetLocalStreamName(), strRemoteFile, llLocalFileSize);
...
/// Executes a command that result in a communication over the data port.
...
bool CFTPClient::ExecuteDatachannelCommand(const CCommand& crDatachannelCmd, const tstring& strPath, const CRepresentation& representation,
	bool fPasv, size_t dwByteOffset, ITransferNotification& Observer) const
{
...
/// @param[in] strPath Parameter for the command usually a path.
/// @param[in] dwByteOffset Server marker at which file transfer is to be restarted.
bool CFTPClient::OpenActiveDataConnection(IBlockingSocket& sckDataConnection, const CCommand& crDatachannelCmd, const tstring& strPath, size_t dwByteOffset) const
{
	if (!crDatachannelCmd.IsDatachannelCommand())
	{
		ASSERT(false);
...
/// @param[in] strPath Parameter for the command usually a path.
/// @param[in] dwByteOffset Server marker at which file transfer is to be restarted.
bool CFTPClient::OpenPassiveDataConnection(IBlockingSocket& sckDataConnection, const CCommand& crDatachannelCmd, const tstring& strPath, size_t dwByteOffset) const
{
	if (!crDatachannelCmd.IsDatachannelCommand())
	{
		ASSERT(false);
...
bool CFTPClient::ReceiveData(ITransferNotification& Observer, IBlockingSocket& sckDataConnection) const
{
	try
	{
		...
		long long llTotalBytes = iNumRead;
		while (!m_fAbortTransfer && iNumRead != 0)
		{
			...
			llTotalBytes += iNumRead;
		}

		for (TObserverSet::const_iterator it2 = m_setObserver.begin(); it2 != m_setObserver.end(); it2++)
			(*it2)->OnEndReceivingData(llTotalBytes);
...
bool CFTPClient::SendCommand(const CCommand& Command, const CArg& Arguments) const
{
	...
	try
	{
		...
		const std::string strCommand = CCnv::ConvertToString(Command.AsString(Arguments), m_fUtf8) + "\r\n";
...
bool CFTPClient::GetSingleResponseLine(tstring& strResponse) const
{
	...
	try
	{
		...
		strResponse = CCnv::ConvertToTString(m_qResponseBuffer.front().c_str(), m_fUtf8);
...
bool CFTPClient::GetIpAddressFromResponse(const tstring& strResponse, ULONG& ulIpAddress, USHORT& ushPort) const
{
	...
	for (tstring::const_iterator it = strResponse.begin(); it != strResponse.end(); ++it)
	{
		...
		case state3:
			...
			break;
		case state4:
			break; // some compilers complain if not all enumeration values are listed
		}
	}
...
int CFTPClient::Abort() const
{
	...
	// JJ: changed to RFC style aborting process
	try
	{
		for (TObserverSet::const_iterator it = m_setObserver.begin(); it != m_setObserver.end(); it++)
			(*it)->OnSendCommand(CCommand::ABOR(), CArg());

		// Sending ABOR - RFC style:
		// - packet 1: 0xFF 0xF4 (normal)
		// - packet 2: 0xFF 0xF2 (urgent)
		// - packet 3: ABOR 0x0D 0x0A (normal)
		SOCKET s(*m_apSckControlConnection);
		char buffer[6];

		buffer[0] = '\xFF';
		buffer[1] = '\xF4';
		if (send(s, buffer, 2, 0) == SOCKET_ERROR) throw CBlockingSocketException(_T("Send"));

		buffer[0] = '\xFF';
		buffer[1] = '\xF2';
		if (send(s, buffer, 2, MSG_OOB) == SOCKET_ERROR) throw CBlockingSocketException(_T("Send"));

		memcpy(buffer, "ABOR\r\n", 6);
		if (send(s, buffer, 6, 0) == SOCKET_ERROR) throw CBlockingSocketException(_T("Send"));
...
/// @return see return values of CFTPClient::SimpleErrorCheck
int CFTPClient::RepresentationType(const CRepresentation& representation, size_t dwSize) const
{
...
/// @param[in] dwSize Indicates Bytesize for type LocalByte.
/// @return see return values of CFTPClient::SimpleErrorCheck
int CFTPClient::_RepresentationType(const CRepresentation& representation, size_t dwSize) const
{
...
/// @return see return values of CFTPClient::SimpleErrorCheck
int CFTPClient::MakeDirectoryFtp(const tstring& strDirectory) const
{
	ASSERT(!strDirectory.empty());
	CReply Reply;
...
/// @return see return values of CFTPClient::SimpleErrorCheck
int CFTPClient::RemoveDirectoryFtp(const tstring& strDirectory) const
{
	ASSERT(!strDirectory.empty());
	CReply Reply;
...
///                       is to be restarted.
/// @return see return values of CFTPClient::SimpleErrorCheck
int CFTPClient::Restart(size_t dwPosition) const
{
	CReply Reply;
...
/// @return see return values of CFTPClient::SimpleErrorCheck
int CFTPClient::FileSize(const tstring& strPath, long long& llSize) const
{
	CReply Reply;
	if (!SendCommand(CCommand::SIZE(), strPath, Reply))
		return FTP_ERROR;
	llSize = CCnv::TStringToLongLong(Reply.Value().substr(4).c_str());
	return SimpleErrorCheck(Reply);
}

/// Executes the FTP command XCRC
/// Calculate checksum of a file.
/// @param[in] Pathname of a file.
/// @param[out] CRC32 hash of the file specified in pathname.
/// @return see return values of CFTPClient::SimpleErrorCheck
int CFTPClient::FileChecksum(const tstring& strPath, DWORD& crc32) const
{
	CReply Reply;
	if (!SendCommand(CCommand::XCRC(), strPath, Reply))
		return FTP_ERROR;
	crc32 = (DWORD)CCnv::TStringToLong(Reply.Value().substr(4).c_str());
	return SimpleErrorCheck(Reply);
}

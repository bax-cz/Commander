...
bool CBlockingSocketException::GetErrorMessage(LPTSTR lpstrError, UINT nMaxError, PUINT /*pnHelpContext = NULL*/)
{
	...
	else
	{
		if (m_strMessage.size() + 18 > nMaxError)
			return false;
		tsprintf(lpstrError, nMaxError, _T("%s Error (0x%08X): %s"), m_strMessage.c_str(), m_nError,
			nsHelper::CError::GetErrorDescription(m_nError).c_str());
	}
	return true;
}
...
tstring CBlockingSocketException::GetErrorMessage(PUINT /*pnHelpContext = NULL*/)
{
	...
	GetErrorMessage(szBuffer, ARRAYSIZE(szBuffer));
	return szBuffer;
}
...
{
	return std::make_unique<CBlockingSocket>();
}

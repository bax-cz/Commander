...
bool CFTPListParser::GetLongLong(LPCTSTR pszLongLong, int iLength, long long& llResult) const
{
	tstring strLongLong(pszLongLong, iLength);

	LPTSTR pszEndPtr = NULL;
	llResult = tcstoll(strLongLong.c_str(), &pszEndPtr, 10);
	...
	for (int j = 1; j < iLength; ++j)
	{
		...
		if (pszLine[j] == _T(','))
		{
			...
			case _T('s'):
				...
				if (!GetLongLong(pszLine + i + 1, j - i - 1, ftpFileStatus.Size()))
...
bool CFTPListParser::ParseUNIXStyleListing(CFTPFileStatus& ftpFileStatus, LPCTSTR pszLine, int iLength)
{
	...
	for (int j = 1; j < iLength; ++j)
	{
		if (pszLine[j] == _T(' ') && pszLine[j - 1] != _T(' '))
		{
			...
			case 4: // getting tentative size
				if (!GetLongLong(pszLine + i, j - i, llSize))
				{
					llSize = -1;
	...
	ftpFileStatus.Size() = llSize;
	ftpFileStatus.SizeType() = CFTPFileStatus::stBinary;

	// handle links - why the need for getting rid of the target??
	if (pszLine[0] == _T('l'))
	{
...
bool CFTPListParser::ParseMSDOSListing(CFTPFileStatus& ftpFileStatus, LPCTSTR pszLine, int iLength)
{
	...
	else
	{
		i = j;
		while (pszLine[j] != _T(' ')) if (++j == iLength) return false;
		if (!GetLongLong(pszLine + i, j - i, ftpFileStatus.Size()))
			ftpFileStatus.Size() = -1;
		ftpFileStatus.SizeType() = CFTPFileStatus::stBinary;
		ftpFileStatus.RetrPossible() = true;
	}

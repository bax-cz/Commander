...
CLogonInfo::CLogonInfo(const tstring& strHostname, USHORT ushHostport, const tstring& strUsername,
	const tstring& strPassword, const tstring& strAccount, bool utf8) :
...
CLogonInfo::CLogonInfo(const tstring& strHostname, USHORT ushHostport, const tstring& strUsername, const tstring& strPassword,
	const tstring& strAccount, bool utf8, const tstring& strFwHostname, const tstring& strFwUsername,
...
class CCommand::CCmd2Info : private std::map<TCommandEnum, std::shared_ptr<CExtendedInfo>>
{
	CCmd2Info();
	...
	void Insert(TCommandEnum enCommand, const tstring& strServerString, const tstring& strCompleteServerStringSyntax, UINT uiNumberOfParameters,
		UINT uiNumberOfOptionalParameters, TSpecificationEnum enSpecification, TTypeEnum enType)
	{
		insert(std::make_pair(enCommand, std::make_shared<CExtendedInfo>(CExtendedInfo(strServerString, strCompleteServerStringSyntax, uiNumberOfParameters,
...
// JJ: a hack to prolong the life of CCmd2Info::TheOneAndOnly instance - so it's not destructed before CFTPClient's
auto g_dummy_ftp_command = CCommand::NOOP().AsString();
...
CCommand::CCmd2Info::CCmd2Info()
{
	...
	Insert(cmdFEAT, _T("FEAT"), _T("FEAT <CRLF>"), 0, 0, RFC3659, NonDatachannel);
	...
	Insert(cmdLSTA, _T("LIST -a"), _T("LIST -a [<SP> <pathname>] <CRLF>"), 1, 1, Unknown, DatachannelRead); // list all
	...
	Insert(cmdXCRC, _T("XCRC"), _T("XCRC <SP> <pathname> <CRLF>"), 1, 0, Unknown, NonDatachannel);
}

...
UnicodeString RandomSeedFileName;
...
void /*__fastcall*/ PuttyInitialize()
{
	...
	RandomSeedFileName = FCS::inst().getTempPath() + L"\\PUTTY.RND";
...
void /*__fastcall*/ PuttyFinalize()
{
	...
	if (DeleteRandomSeedOnExit())
	{
		...
		DeleteFile(/*ApiPath(Configuration->*/RandomSeedFileName.c_str()/*)*/);
	}
...
//---------------------------------------------------------------------------
bool IsPasswordOrPassphrasePrompt(TPromptKind Kind, prompts_t *p)
{
	return
		(p->n_prompts == 1) /*&& FLAGCLEAR(int(Prompts->Objects[0]), pupEcho)*/ &&
		((Kind == pkPassword) || (Kind == pkPassphrase) || (Kind == pkKeybInteractive) ||
		(Kind == pkTIS) || (Kind == pkCryptoCard));
}
//---------------------------------------------------------------------------
bool IsPasswordPrompt(TPromptKind Kind, prompts_t *p)
{
	return
		IsPasswordOrPassphrasePrompt(Kind, p) &&
		(Kind != pkPassphrase);
}
//---------------------------------------------------------------------------
static SeatPromptResult get_userpass_input(Seat *seat, prompts_t *p)
{
	...
	SeatPromptResult Result = p->n_prompts ? SPR_USER_ABORT : SPR_OK; // TODO: on MacOSX an empty prompt
//	auto Prompts = std::make_unique<TStrings>();//new TStringList();
//	auto Results = std::make_unique<TStrings>();//new TStringList();
	try
	{
		UnicodeString Name = UTF8ToString(p->name);
		TPromptKind PromptKind = SecureShell->IdentifyPromptKind(Name);
		...
		for (int Index = 0; Index < int(p->n_prompts); Index++)
		{
			...
			// check whether user has not canceled login process
			if (SecureShell->pWorker && !SecureShell->pWorker->isRunning())
				return SPR_USER_ABORT;

			UnicodeString res;
			if (SecureShell->FAuthenticating && SecureShell->GetStoredCredentialsTried())
			{
				CTextInput::CParams params(
					Name.get(),
					S.get(),
					IsPasswordPrompt(PromptKind, p) ?
					SecureShell->FSessionData->FPassphrase.get() :
					SecureShell->FSessionData->FUserName.get(),
					true, IsPasswordPrompt(PromptKind, p));

				res = MiscUtils::showTextInputBox(params, SecureShell->pWorker->getHwnd());

				// update session data
				if (IsPasswordPrompt(PromptKind, p))
					SecureShell->FSessionData->FPassphrase = res;
				else
					SecureShell->FSessionData->FUserName = res;

				Result = res.IsEmpty() ? SPR_USER_ABORT : SPR_OK;
			}
			else
			{
				if (IsPasswordPrompt(PromptKind, p))
				{
					SecureShell->FStoredPassphraseTried = true;
					res = SecureShell->FSessionData->FPassphrase;
				}
				else
					res = SecureShell->FSessionData->FUserName;

				Result = SPR_OK;
			}

			prompt_set_result(Prompt, UTF8String(res).c_str());

			/*Prompts->push_back(S);//AddObject(S, (TObject *)(FLAGMASK(Prompt->echo, pupEcho)));
			...
			Instructions, p->instr_reqd, Prompts.get(), Results.get()))
		{
			for (int Index = 0; Index < int(p->n_prompts); Index++)
			{
				...
				if (p->utf8)
				{
					S = RawByteString(UTF8String(Results->at(Index)));
				}
				else
				{
					S = RawByteString(AnsiString(Results->at(Index)));
				}
...
static void nonfatal(Seat *, const char *message)
{
	...
	DebugFail();
	AppLog(UnicodeString(AnsiString(message)).c_str());
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
extern "C" void putty_registry_pass(bool enable)
{
	if (enable)
	{
...
/*HKEY open_regkey_fn_winscp(bool Create, bool Write, HKEY Key, const char *Path, ...)
{
	...
	else if (PuttyRegistryMode == prmRedirect)
	{
		...
		size_t PuttyKeyLen = OriginalPuttyRegistryStorageKey.Length();
...
UnicodeString GetPublicKeyLine(const UnicodeString& FileName, UnicodeString& Comment, bool& HasCertificate)
{
	...
	UnicodeString Result = FORMAT(L"%ls %ls %ls", Algorithm.c_str(), PublicKeyBase64.c_str(), Comment.c_str());
	return Result;
}
//---------------------------------------------------------------------------
	...
	const UnicodeString& Str, UnicodeString& Error, int& ErrorStart, int& ErrorLen)
{
	...
	if (!Result)
	{
		...
		sfree(ErrorMsg);
		ErrorStart = static_cast<int>(static_cast<const char*>(ErrorLoc.ptr) - StrPtr);
		ErrorLen = static_cast<int>(ErrorLoc.len);
	}
	return Result;
}
//---------------------------------------------------------------------------
...
{
	std::unique_ptr<TStrings> Result(new TStrings());
...
TStrings *SshMacList()
{
	...
	for (int Index = 0; Index < Count; Index++)
	{
		...
		if (!ETMName.IsEmpty())
		{
			S = FORMAT(L"%ls (%ls)", S.c_str(), ETMName.c_str());
		}
		Result->push_back(S);
	}
	return Result.release();
...
void WritePuttySettings(THierarchicalStorage *Storage, const UnicodeString& ASettings)
{
	...
	std::unique_ptr<TStrings> Settings(new TStrings());
	...
	{
		UnicodeString Name = Settings->at(Index);

...
//	TSessionData __fastcall *Clone();
	inline void /*__fastcall*/ Default() {
		...
		FSelected = false;
		FModified = false;
		FSource = TSessionSource::ssNone;
		FSaveOnly = false;
		// add also to TSessionLog::AddStartupInfo()
	}
	inline void /*__fastcall*/ DefaultSettings() {
		FHostName = L"";
		...
		{
			FHostKeys[Index] = DefaultHostKeyList[Index];
		}
		for (int Index = 0; Index < GSSLIB_COUNT; Index++)
		{
			FGssLib[Index] = DefaultGssLibList[Index];
		}
		FGssLibCustom = L"";
		FPublicKeyFile = EmptyStr;
		FDetachedCertificate = EmptyStr;
		FPassphrase = L"";
		FPuttyProtocol = L"";
		FTcpNoDelay = false;
		...
		FNote = L"";
		FWinTitle = L"";
		FInternalEditorEncoding = -1;

		FEncryptKey = UnicodeString();

		FWebDavLiberalEscaping = false;
		FWebDavAuthLegacy = false;

		FProxyMethod = TProxyMethod::pmNone;
		FProxyHost = L"proxy";
		FProxyPort = ProxyPortNumber;
		FProxyUsername = L"";
		FProxyPassword = L"";
		FProxyTelnetCommand = L"connect %host %port\\n";
		FProxyLocalCommand = L"";
		FProxyDNS = asAuto;
		FProxyLocalhost = false;

		for (unsigned int Index = 0; Index < LENOF(FBugs); Index++)
		{
			FBugs[(TSshBug)Index] = asAuto;
		}
		...
		FShell = L""; //default shell
		FReturnVar = L"";
		FExitCode1IsError = false;
		FClearAliases = true;
		FUnsetNationalVars = true;
		FListingCommand = L"ls -la";
		...
		FUsePosixRename = false;

		for (unsigned int Index = 0; Index < LENOF(FSFTPBugs); Index++)
		{
			FSFTPBugs[(TSftpBug)Index] = asAuto;
		}
		...
		FFtpAccount = L"";
		FFtpPingInterval = 30;
		FFtpPingType = fptDummyCommand;
		FFtpTransferActiveImmediately = asAuto;
		FFtps = ftpsNone;
		FMinTlsVersion = tlsDefaultMin;
		FMaxTlsVersion = tlsMax;
		FCompleteTlsShutdown = asAuto;
		FFtpListAll = asAuto;
		FFtpHost = asAuto;
	...
	inline void ExpandEnvironmentVariables() {
		FHostName = GetHostNameExpanded();
		FUserName = GetUserNameExpanded();
		FPublicKeyFile = bcb::ExpandEnvironmentVariables(FPublicKeyFile);
		FDetachedCertificate = bcb::ExpandEnvironmentVariables(FDetachedCertificate);
	}
	inline UnicodeString GetHostNameExpanded() {
		return bcb::ExpandEnvironmentVariables(FHostName);
	}
	inline bool HasS3AutoCredentials() {
		return (FFSProtocol == fsS3) && FS3CredentialsEnv;
	}
	inline UnicodeString GetUserNameExpanded() {
		UnicodeString Result = bcb::ExpandEnvironmentVariables(FUserName);
		if (Result.IsEmpty() && HasS3AutoCredentials())
		{
		//	Result = S3EnvUserName(FS3Profile);
		}
		return Result;
	}
	inline UnicodeString ResolvePublicKeyFile()	{
		UnicodeString Result = FPublicKeyFile;
		if (Result.IsEmpty())
		{
		//	Result = Configuration->DefaultKeyFile;
		}
		// StripPathQuotes should not be needed as we do not feed quotes anymore
		Result = StripPathQuotes(bcb::ExpandEnvironmentVariables(Result));
		return Result;
	}

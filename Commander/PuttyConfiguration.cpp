...
{
	return FORMAT(L"%ls:%ls", SiteKey.c_str(), FingerprintType.c_str());
}
//---------------------------------------------------------------------------
...
UnicodeString TConfiguration::GetVersionStrHuman()
{
	...
	try
	{
		...
		UnicodeString Result = FORMAT(L"%ls (%ls)", FullVersion.c_str(), DateStr.c_str());
	...
	TStoredSessionList *Sessions, UnicodeString& Error)
{
	...
	{
		Error = FORMAT(L"%ls\n(%ls)", E.Message.c_str(), KnownHostsFile.c_str());
	}

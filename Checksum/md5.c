...
void MD5_Core_Init(MD5_CoreState * s)
{
	s->h[0] = 0x67452301;
	s->h[1] = 0xefcdab89;
	s->h[2] = 0x98badcfe;
...
void MD5_Block(MD5_CoreState * s, uint32 * block)
{
	uint32 a, b, c, d;
...
void MD5_Init(struct MD5_Context *s)
{
...
void MD5_Update(struct MD5_Context *s, unsigned char const *p, unsigned len)
{
...
void MD5_Final(unsigned char output[16], struct MD5_Context *s)
{
	int i;
	unsigned pad;
	unsigned char c[64];

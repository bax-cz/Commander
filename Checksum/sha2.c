...
void Sha512_Update(Sha512_State *s, unsigned char *buf, unsigned int len) {
    ...
    if (s->blkused && s->blkused+len < SHA512_BLOCK_SIZE) {
        /*
         ...
         */
        while (s->blkused + len >= SHA512_BLOCK_SIZE) {
            memcpy(s->block + s->blkused, q, SHA512_BLOCK_SIZE - s->blkused);
            q += SHA512_BLOCK_SIZE - s->blkused;
            len -= SHA512_BLOCK_SIZE - s->blkused;
...
void Sha512_Final(Sha512_State *s, unsigned char *digest) {
    int i;
    int pad;
    unsigned char c[SHA512_BLOCK_SIZE];
    uint64_t lenhi, lenlo;
	unsigned char data[8];

    if (s->blkused >= SHA512_BLOCK_SIZE - 16)
        pad = (SHA512_BLOCK_SIZE - 16) + SHA512_BLOCK_SIZE - s->blkused;
    else
        pad = (SHA512_BLOCK_SIZE - 16) - s->blkused;
    ...
    c[0] = 0x80;
    //put_data(s, &c, pad);
	Sha512_Update(s, c, (unsigned int)pad);

    //put_uint64(s, lenhi);
    PUT_64BIT_MSB_FIRST(data, lenhi);
	Sha512_Update(s, data, sizeof(data));

    //put_uint64(s, lenlo);
	PUT_64BIT_MSB_FIRST(data, lenlo);
	Sha512_Update(s, data, sizeof(data));

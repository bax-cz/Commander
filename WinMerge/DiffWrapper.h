...
public:
	...
	void SetPaths(const /*PathContext*/std::vector<String>& files, bool tempPaths);
	...
	void SetCompareFiles(const /*PathContext*/std::vector<String>& originalFile);
...
private:
	...
	std::vector<String>/*PathContext*/ m_files; /**< Full path to diff'ed file. */
	//PathContext m_alternativePaths; /**< file's alternative path (may be relative). */
	std::vector<String>/*PathContext*/ m_originalFile; /**< file's original (NON-TEMP) path. */
 ...
 */
inline void CDiffWrapper::SetCompareFiles(const /*PathContext*/std::vector<String>& originalFile)
{

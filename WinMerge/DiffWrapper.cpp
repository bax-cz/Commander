 ...
 */
void CDiffWrapper::SetPaths(const /*PathContext*/ std::vector<String>& tFiles,
		bool tempPaths)
{
	m_files = tFiles;
...
{
	/*PathContext*/std::vector<String> aFiles = m_files;
	/*int file;
	...
	// Create patch file
	if (!m_status.bBinaries && m_bCreatePatchFile && aFiles./*GetSize*/size() == 2)
	{
...
void CDiffWrapper::AddDiffRange(DiffList *pDiffList, unsigned begin0, unsigned end0, unsigned begin1, unsigned end1, OP_TYPE op)
{
	...
	{
		/*AppError*/MessageBox(NULL, StringUtils::convert2W(e.what()).c_str(), L"Error", MB_ICONEXCLAMATION | MB_OK);
	}
}

...
bool DiffFileData::DoOpenFiles()
{
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		m_inf[i].name = _strdup(StringUtils::convert2A(m_sDisplayFilepath[i]).c_str());
		...
		if (m_inf[i].desc == 0)
		{
			_wsopen_s(&m_inf[i].desc, /*TFile(*/PathUtils::getExtendedPath(m_FileLocation[i]/*.filepath).wpath(*/).c_str(),
